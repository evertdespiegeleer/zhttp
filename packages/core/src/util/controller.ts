...
import { type AnyEndpoint, endpointToExpressHandler } from './endpoint.js'
...
export const bindControllerToApp = (controller: Controller, app: Application) => {
  const controllerBeforeMiddlewares = controller.getMiddlewares().filter((m) => m.type === MiddlewareTypes.BEFORE)
  const controllerAfterMiddlewares = controller.getMiddlewares().filter((m) => m.type === MiddlewareTypes.AFTER)
  ...
  controller.getEndpoints().forEach((endpoint) => {
    app[endpoint.getMethod()](
      ...
      ...(endpoint.getMiddlewares() ?? []).filter((m) => m.type === MiddlewareTypes.BEFORE).map((m) => m.handler),

      // Endpoint handler
      endpointToExpressHandler(endpoint),

      // Endpoint after middleware
      ...(endpoint.getMiddlewares() ?? []).filter((m) => m.type === MiddlewareTypes.AFTER).map((m) => m.handler),
